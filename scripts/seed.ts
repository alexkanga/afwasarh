...
function generatePhone(): string { return `+33${randomInt(600000000, 799999999)}`; }
...
async function main() {
  ...
  for (const pos of positions) {
    const created = await prisma.position.create({
      data: { title: pos.title, baseSalary: pos.baseSalary, description: `Poste de ${pos.title}` }
    });
    posMap[pos.title] = { id: created.id, baseSalary: pos.baseSalary };
  }

  console.log('👥 Création des employés...');
  const employees: { id: string; dept: string; position: string; status: EmployeeStatus; hireDate: Date }[] = [];
  ...
  for (const dist of contractDistribution) {
    for (let i = 0; i < dist.count; i++) {
      ...
      if (dist.type === ContractType.CDD) endDate = new Date('2026-08-31');
      else if (dist.type === ContractType.INTERIM) endDate = new Date('2026-04-30');
      else if (dist.type === ContractType.APPRENTICESHIP) { endDate = new Date('2027-08-31'); workHours = 28; }
      else if (dist.type === ContractType.PART_TIME) workHours = 24;

      await prisma.contract.create({
        data: { employeeId: employee.id, type: dist.type, startDate: hireDate, endDate, monthlySalary: Math.round(salaryVar), workHours, isActive: true }
      });

      await prisma.movement.create({
        data: { employeeId: employee.id, type: MovementType.HIRE, effectiveDate: hireDate, toDepartment: dept, toPosition: pos.title }
      });
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const employee = await prisma.employee.create({
      data: {
        ...
        departmentId: deptMap[dept],
        positionId: posMap[positions[i].title]?.id,
      }
    });

    await prisma.contract.create({
      data: { employeeId: employee.id, type: ContractType.CDI, startDate: hireDate, endDate: terminationDate, monthlySalary: 3500, workHours: 35, isActive: false }
    });

    await prisma.movement.create({
      data: { employeeId: employee.id, type: MovementType.HIRE, effectiveDate: hireDate, toDepartment: dept, toPosition: positions[i].title }
    });
    await prisma.movement.create({
      data: { employeeId: employee.id, type: MovementType.TERMINATION, effectiveDate: terminationDate, reason: ['Démission', 'Fin de contrat', 'Licenciement'][i % 3] }
    });
  }
  ...
  for (const train of trainings) {
    const created = await prisma.training.create({
      data: { ...train, startDate: randomDate(new Date('2026-01-15'), new Date('2026-06-15')), location: 'Paris' }
    });
  ...
  for (const emp of employees) {
    ...
    for (let i = 0; i < num; i++) {
      const title = Object.keys(trainingMap)[randomInt(0, Object.keys(trainingMap).length - 1)];
      ...
      await prisma.employeeTraining.create({
        data: {
          ...
          status,
          hoursCompleted: status === TrainingStatus.COMPLETED ? 14 : status === TrainingStatus.IN_PROGRESS ? 7 : 0,
        }
      });
    }
  }
  ...
  for (const emp of employees) {
    for (let i = 0; i < randomInt(1, 3); i++) {
      const type = randomElement(Object.values(AbsenceType));
      ...
      await prisma.absence.create({
        data: {
          employeeId: emp.id, type, startDate, endDate, days,
    ...
    for (const month of months) {
      if (month >= emp.hireDate) {
        ...
        await prisma.salary.create({
          data: {
            employeeId: emp.id, month, baseSalary, bonus, overtime: 0, deductions: 0, socialCharges,
  ...
  console.log(`📊 Résumé: ${await prisma.department.count()} départements, ${await prisma.position.count()} postes, ${await prisma.employee.count()} employés, ${await prisma.contract.count()} contrats`);
}
